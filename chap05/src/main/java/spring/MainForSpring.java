...
import spring.config.ApplicationConfigurationImport;
...
public class MainForSpring {
	...
	public MainForSpring() {
		...
		applicationContext = new AnnotationConfigApplicationContext(ApplicationConfigurationImport.class);
	}
	...
	public void processListCommand() {//Constructor DI 방식
		PrintEntireMemberService printEntireMemberService = applicationContext.getBean("listPrinter", PrintEntireMemberService.class);
	...
	public void processFindCommand(String[] args) {//Setter DI 방식
		...
		PrintSingleMemberService printSingleMemberService = applicationContext.getBean("infoPrinter", PrintSingleMemberService.class);
		MemberPrinter memberPrinter = applicationContext.getBean("memberPrinter", MemberPrinter.class);
		try {
	...
	public void processNewCommand(String[] args) {
		...
		MemberRegisterService memberRegisterService = applicationContext.getBean(MemberRegisterService.class);
	...
	public void processChangeCommand(String[] args) {
		...
		ChangePasswordService changePasswordService = applicationContext.getBean(ChangePasswordService.class);
		try {
	...
	public void processVersionCommand() {
		VersionPrinter versionPrinter = applicationContext.getBean("versionPrinter", VersionPrinter.class);//해당타입 빈 객체가 하나만 존재하기 때문에 "versionPrinter" 매개변수 생략가능
